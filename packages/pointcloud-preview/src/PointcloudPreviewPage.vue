...
const {
  ...
  loadMeasurements,
  resetAnalysisState,
} = useViewerMeasurements({ assetId, panelRef: pointcloudPanelRef })
...
const pageTitle = '点云全屏预览'
const emptyText = '请返回项目的扫描点云列表，选择已就绪的点云进行预览。'
...
function applyPanelSettings() {
  const panel = pointcloudPanelRef.value
  if (!panel) {
    return
  }
...
async function loadPointcloudAppearance() {
  ...
  try {
    const response = await getAssetDetail(currentAssetId)
    ...
    if (token === pointcloudAppearanceLoadToken) {
      console.warn('[PointcloudPreview] 读取点云显示颜色失败', error)
    }
  }
}
...
async function loadPointcloudResources() {
  ...
  try {
    const [detailResponse, preprocessResponse] = await Promise.all([
      getAssetDetail(currentAssetId),
      getPointcloudPreprocess(currentAssetId),
    ])
    ...
    try {
      const detailResponse = await getAssetDetail(currentAssetId)
      if (token !== pointcloudResourceLoadToken) return
      pointcloudSourceUrl.value = detailResponse.data?.tilesetUrl || ''
    } catch {
...
async function togglePointcloudFullscreen() {
  ...
  try {
    ...
    console.warn('[PointcloudPreview] 切换全屏失败', error)
  }
}
...
<template>
  <section class="pointcloud-preview-page">
