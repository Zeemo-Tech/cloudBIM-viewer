...
async function loadBimModel(assetId: number) {
  ...
  dracoLoader.setDecoderPath(getCloudBimRuntime().dracoDecoderPath)
  dracoLoader.preload()
...
async function loadPointcloudModel(assetId: number, expectedToken: number) {
  ...
  applyPointcloudSourceVisibility()
...
function setPointSize(size: number) {
  ...
  // 覆盖层点云与主点云保持同一点大小，避免切换视图时尺寸跳变。
  if (pointcloudOverlay) setPointcloudOverlaySize(nextSize)
}
...
// ---------------------------
// 外部点云覆盖层
// 功能包（去噪结果、自定义分析产物等）自己准备几何与顶点颜色，这里只负责把它放到
// 与点云切片相同的坐标系里，并提供显隐/点大小/刷新入口。
// 几何的所有权属于调用方：`clearPointcloudOverlay` 不会 dispose 传入的 geometry。
// ---------------------------
let pointcloudOverlay: THREE.Points | null = null
let pointcloudOverlayOrigin: [number, number, number] = [0, 0, 0]
let pointcloudSourceVisible = true

function pointcloudOverlayAnchor() {
  // 覆盖层几何与切片内容同坐标系，因此对齐到 `tileset.group`（而不是带姿态修正的
  // wrapper），这样调用方只需给出产物侧的 `previewOrigin` 平移。
  return tileset?.group ?? pointcloudWrapper ?? null
}

function syncPointcloudOverlayTransform() {
  const points = pointcloudOverlay
  if (!points) return
  const anchor = pointcloudOverlayAnchor()
  if (anchor) {
    anchor.updateWorldMatrix(true, false)
    points.matrix.copy(anchor.matrixWorld)
  } else {
    points.matrix.identity()
  }
  points.matrix.multiply(new THREE.Matrix4().makeTranslation(...pointcloudOverlayOrigin))
  points.matrixAutoUpdate = false
  points.updateMatrixWorld(true)
}

function applyPointcloudSourceVisibility() {
  if (tileset) tileset.group.visible = pointcloudSourceVisible
}

/**
 * 注入覆盖点云。`origin` 与产物侧 `previewOrigin` 语义一致：覆盖层几何原点相对点云
 * 组原点的平移；传 `null` 只做清理。
 */
function setPointcloudOverlay(options: {
  ...
  pointcloudOverlayOrigin = options?.origin
    ? [options.origin[0], options.origin[1], options.origin[2]]
    : [0, 0, 0]
  const material = new THREE.PointsMaterial({
    size: options?.size ?? pointSize,
    sizeAttenuation: false,
    vertexColors: Boolean(geometry.getAttribute('color')),
  })
  const points = new THREE.Points(geometry, material)
  points.name = 'pointcloud-overlay'
  points.visible = options?.visible ?? true
  points.frustumCulled = false
  points.matrixAutoUpdate = false
  pointcloudOverlay = points
  scene?.add(points)
  syncPointcloudOverlayTransform()
  requestRender()
}

/** 调用方改写覆盖层几何（颜色 / 索引 / drawRange / 顶点数）之后通知查看器。 */
function refreshPointcloudOverlay() {
  const points = pointcloudOverlay
  if (!points) return
  const material = points.material as THREE.PointsMaterial
  const hasColor = Boolean(points.geometry.getAttribute('color'))
  if (material.vertexColors !== hasColor) {
    material.vertexColors = hasColor
    material.needsUpdate = true
  }
  const position = points.geometry.getAttribute('position')
  if (position) points.geometry.setDrawRange(0, position.count)
  points.geometry.computeBoundingSphere()
  syncPointcloudOverlayTransform()
  requestRender()
}

function setPointcloudOverlayVisible(visible: boolean) {
  if (pointcloudOverlay) pointcloudOverlay.visible = visible
  requestRender()
}

function setPointcloudOverlaySize(size: number) {
  const points = pointcloudOverlay
  if (!points) return
  const material = points.material as THREE.PointsMaterial
  material.size = size
...
function clearPointcloudOverlay() {
  const points = pointcloudOverlay
  if (!points) return
  pointcloudOverlay = null
  points.removeFromParent()
  ;(points.material as THREE.Material).dispose()
  requestRender()
}

/** 隐藏 / 显示主点云切片内容，用于「原始点云」与「覆盖层结果」二选一展示。 */
function setPointcloudSourceVisible(visible: boolean) {
  pointcloudSourceVisible = visible
  applyPointcloudSourceVisibility()
  requestRender()
}
...
defineExpose({
  ...
  setPointcloudOverlayVisible,
  setPointcloudOverlaySize,
  refreshPointcloudOverlay,
  clearPointcloudOverlay,
  setPointcloudSourceVisible,
