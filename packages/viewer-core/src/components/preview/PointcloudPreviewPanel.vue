...
defineExpose({
  ...
  // 外部点云覆盖层：功能包自行准备几何与顶点颜色，查看器只负责摆放与显隐。
  setPointcloudOverlay: (options: {
    geometry: BufferGeometry | null
    size?: number
    origin?: readonly [number, number, number]
    visible?: boolean
  } | null) => viewerRef.value?.setPointcloudOverlay(options),
  setPointcloudOverlayVisible: (visible: boolean) => viewerRef.value?.setPointcloudOverlayVisible(visible),
  setPointcloudOverlaySize: (size: number) => viewerRef.value?.setPointcloudOverlaySize(size),
  refreshPointcloudOverlay: () => viewerRef.value?.refreshPointcloudOverlay(),
  clearPointcloudOverlay: () => viewerRef.value?.clearPointcloudOverlay(),
  setPointcloudSourceVisible: (visible: boolean) => viewerRef.value?.setPointcloudSourceVisible(visible),
