...
async function loadProjectAssets(project: ProjectSummary, force = false) {
  ...
  try {
    ...
    projectAssets[project.id] = response.data?.list || []
  } catch (error) {
          ...
          <template #default="{ row }">
            <div class="asset-child-panel" v-loading="assetLoading[row.id]">
              ...
              <el-table class="asset-child-table" :data="projectAssets[row.id] || []" row-key="id" empty-text="该项目暂未上传文件">
                <el-table-column label="文件名称" min-width="260"><template #default="{ row: asset }"><div class="asset-name-cell"><span class="asset-type-icon" :class="asset.type">{{ asset.type === 'bim' ? 'BIM' : 'PC' }}</span><span>{{ asset.sourceName }}</span></div></template></el-table-column>
                <el-table-column label="类型" width="120"><template #default="{ row: asset }">{{ asset.type === 'bim' ? 'BIM 模型' : '点云文件' }}</template></el-table-column>
