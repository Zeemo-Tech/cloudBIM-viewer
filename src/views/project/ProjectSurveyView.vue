...
function handleEmptyAction() { if (assets.value.length) resetFilters(); else uploadVisible.value = true }
...
function openAlignment(pointcloud: AssetSummary) {
  ...
  void router.push({ path: '/alignment', query: { projectId: props.projectId, projectName: props.projectName, bimAssetId: bim.id, pointcloudAssetId: pointcloud.id, bimDisplayName: pointcloud.archiveCode ? `${pointcloud.archiveCode} · ${bim.sourceName}` : bim.sourceName, pointcloudDisplayName: pointcloud.sourceName } })
}
...
<template>
  <section class="project-page">
    ...
    <section class="project-table card">
      <h2 class="table-title">实测扫描</h2>
      <div class="table-scroll-region" tabindex="0" aria-label="实测扫描表格，可横向滚动">
        <el-table class="zeemo-table" v-loading="loading" :data="pagedAssets" row-key="id" highlight-current-row>
          <el-table-column label="归档编号" width="150" align="left"><template #default="{ row }"><strong class="archive-code-cell">{{ row.archiveCode || '未归档' }}</strong></template></el-table-column>
          <el-table-column label="文件名称" min-width="280" align="left"><template #default="{ row }"><div class="asset-name-cell"><span class="asset-type-icon pointcloud">点云</span><span :title="row.sourceName">{{ row.sourceName }}</span></div></template></el-table-column>
          <el-table-column prop="building" label="楼栋" width="100" />
          <el-table-column label="构件类型" width="160"><template #default="{ row }">{{ componentText(row.componentType) }}</template></el-table-column>
          <el-table-column label="扫描日期" width="140"><template #default="{ row }">{{ formatScanDate(row.scanDate) }}</template></el-table-column>
          <el-table-column label="关联模型" min-width="220"><template #default="{ row }"><span :class="linkedBim(row) ? 'link-ready' : 'link-missing'">{{ linkedBim(row)?.sourceName || '未匹配 IFC' }}</span></template></el-table-column>
          <el-table-column label="状态" width="110"><template #default="{ row }"><el-tag size="small" :type="assetStatusType(row.status)">{{ assetStatusText(row.status) }}</el-tag></template></el-table-column>
          <el-table-column label="操作" width="260" fixed="right" class-name="asset-operation-column" label-class-name="asset-operation-column"><template #default="{ row }"><div class="row-actions"><button class="action-button action-button--primary is-align" type="button" title="进入分析" aria-label="进入分析" :disabled="actionLoading || !row.linkedBimId" @click.stop="openAlignment(row)"><el-icon><Aim /></el-icon><span>进入分析</span></button><el-tooltip content="查看实模结果" placement="top"><button class="action-button is-result" type="button" title="查看实模结果" aria-label="查看实模结果" :disabled="actionLoading || !row.linkedBimId" @click.stop="openResult(row)"><el-icon><DataAnalysis /></el-icon></button></el-tooltip><el-tooltip content="预览点云" placement="top"><button class="action-button is-preview" type="button" title="预览点云" aria-label="预览点云" :disabled="row.status !== 'ready'" @click="previewAsset(row)"><el-icon><View /></el-icon></button></el-tooltip><el-tooltip content="删除点云" placement="top"><button class="action-button is-delete" type="button" title="删除点云" aria-label="删除点云" @click="removeAsset(row)"><el-icon><Delete /></el-icon></button></el-tooltip></div></template></el-table-column>
          <template #empty>
            <div class="table-empty">
              <strong>{{ assets.length ? '没有匹配的实测扫描' : '当前项目暂无点云扫描' }}</strong>
    ...
    <NeumorphicPagination v-model:current-page="currentPage" v-model:page-size="pageSize" :total="filteredAssets.length" :page-size-options="[10, 20, 50]" aria-label="实测扫描分页" />
...
.project-header { margin-bottom: var(--spacing-compact); }
.project-table { flex: 0 1 auto; }
.table-scroll-region { min-width: 0; overflow-x: auto; border-radius: var(--radius-md); }
.table-scroll-region:focus-visible { outline: 2px solid var(--border-color-focus); outline-offset: 2px; }
.zeemo-table { flex: 0 0 auto; }
.action-button { width: 40px; height: 40px; flex-basis: 40px; }
.action-button--primary { width: auto; min-width: 104px; display: inline-flex; gap: var(--spacing-sm); padding: 0 var(--spacing-compact); color: var(--bg-card); background: var(--color-primary); box-shadow: var(--shadow-sm); }
.action-button--primary:hover { color: var(--bg-card); background: var(--color-primary-hover); }
.action-button:focus-visible, .toolbar-button:focus-visible, .table-empty button:focus-visible { outline: 2px solid var(--border-color-focus); outline-offset: 2px; }
...
.table-empty button { min-height: var(--control-height); margin-top: var(--spacing-xs); padding: 0 var(--spacing-md); border: 0; border-radius: var(--radius-sm); color: var(--bg-card); background: var(--color-primary); cursor: pointer; }
</style>
