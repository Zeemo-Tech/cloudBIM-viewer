...
def constrained_nearest(
    ...
    fallback_mode: str, chunk_size: int = 8192, solid=None, max_search_distance: float = 0.2,
    ...
    """
    if not math.isfinite(max_search_distance) or not 0.0001 <= max_search_distance <= 0.2:
    ...
    for offset in range(0, count, chunk_size):
        ...
        accepted &= candidate_norm <= max_search_distance
        if half_space_only:
            accepted &= np.einsum("ijk,ik->ij", delta, normals[offset:stop]) >= 0.0
        # The solid boundary is a hard constraint, including nearest fallback.
        safe = np.broadcast_to((normal_length > 1e-8)[:, None], accepted.shape).copy()
        safe &= candidate_norm <= max_search_distance
        if half_space_only:
            safe &= np.einsum("ijk,ik->ij", delta, normal) >= 0.0
        if solid is not None:
            check = safe & (normal_dot < 0.0)
            if fallback_mode != "nearest":
                check &= accepted
            row, col = np.nonzero(check)
            if len(row):
                safe[row, col] &= solid.allows_inward(query[row], candidate[row, col])
        accepted &= safe
        first = np.argmax(accepted, axis=1)
        exists = accepted[np.arange(stop - offset), first]
        if fallback_mode == "nearest":
            fallback = np.argmax(safe, axis=1)
            fallback = np.where(safe[np.arange(stop - offset), fallback], fallback, -1)
            first = np.where(exists, first, fallback)
        else:
