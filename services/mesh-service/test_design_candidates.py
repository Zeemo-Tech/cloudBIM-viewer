...
class DesignCandidateTests(unittest.TestCase):
    def test_fixed_radius_and_observed_span_for_upper_half_cylinder(self):
        points, normals, _ = cylinder((0, 0, .02), (.12, 0, .02), radius=.006, along=30, around=16)
        keep = normals[:, 2] > 0
        result, report = generate_candidates(context(points[keep], normals[keep]), unit(start=(0,0,.02), end=(.12,0,.02)))
        self.assertTrue(result); candidate = result[0]
        self.assertEqual(candidate.model['radius'], .006)
