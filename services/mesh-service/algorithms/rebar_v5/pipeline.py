...
import numpy as np
from ..spatial_keys import integer_row_groups, unique_integer_rows
...
from .chunks import ChunkCache, DerivedCache, memory_budget
from .spatial import SpatialStore, SpatialBudgetExceeded
from .scene import detect_table, table_mask, detect_fixtures, fixture_mask, fixture_candidates, refine_fixture_faces, fixture_rows
from .bolts import detect_bolts, bolt_mask, bolt_candidates
...
from .scheduling import ordered_work, worker_count
...
class Runtime:
    ...
    def __init__(self,p,cache_bytes=None):
        budget=memory_budget() if cache_bytes is None else max(0,int(cache_bytes))
        self.chunks=ChunkCache(budget-budget//8)
        ...
        self.label_chunks={}
        self.transfer_cache=DerivedCache(budget//8)
        self.update_chunks={}
    ...
    def cache_diagnostics(self):
        # Returned manifests outlive Runtime.close(), which clears the caches.
        return {'intermediateCache':dict(self.chunks.stats),
                'displayIndexCache':dict(self.transfer_cache.stats)}
    ...
    def masks(self,records,name="noise"):
        ...
        for key,rows in integer_row_groups(keys):
...
def _denoise_core(job):
    runtime,lo,hi=job;p=runtime.p
    ...
    return records,noise,suspect


def _feature_core(job):
    runtime,lo,hi=job;p=runtime.p
    support=runtime.store.query(lo-p.halo-p.detection_voxel_size,hi+p.halo+p.detection_voxel_size)
    core=np.all((support["xyz"]>=lo)&(support["xyz"]<hi),axis=1)
    ...
    # Preserve each original core and its complete halo, including source-order
    # tie-breaking in the global detection lattice. Only execution is parallel.
    active=np.flatnonzero(~noise)
    voxel=np.floor(support["xyz"][active]/p.detection_voxel_size).astype(np.int64)
    _,first=unique_integer_rows(voxel,return_index=True)
    representatives=active[first];representatives=representatives[core[representatives]]
    keep=np.searchsorted(records["source_index"],support["source_index"][representatives])
    return lo,hi,payload,keep
...
def prepare(context,p):
    ...
    try:
        ...
        workers=worker_count(p, runtime.chunks.budget) if runtime.store.count>=250_000 else 1
        logging.getLogger(__name__).info('V5 spatial execution: %d workers, %d unchanged cores',workers,len(cores))
        jobs=((runtime,lo,hi) for _,lo,hi in cores)
        for records,noise,suspect in ordered_work(_denoise_core,jobs,workers):
        ...
        jobs=((runtime,lo,hi) for _,lo,hi in cores)
        for ordinal,(lo,hi,payload,keep) in enumerate(ordered_work(_feature_core,jobs,workers)):
            path=runtime.path/f"features-{ordinal:06d}.npz"
            runtime.chunks.save(path,**payload)
            ...
            parts.append(payload["xyz"][keep]);indices_parts.append(payload["source_index"][keep])
            feature_parts.append({k:v[keep] for k,v in payload.items() if k.startswith(("surface_","axis_"))})
        ...
        for lo,hi,faces in regions:
            try:
                ...
                for _,core_lo,core_hi in runtime.store.cores():
                    ...
                    yield records["xyz"][valid],local_faces,p,(lower,upper)
                continue
            valid=~runtime.masks(records)
            valid&=~table_mask(records["xyz"],plane,p)
            yield records["xyz"][valid],faces,p,None
    workers=1
    if runtime.store.count>=250_000 and hasattr(runtime,'chunks'):
        reserve=max(0,runtime.chunks.budget-runtime.chunks.stats['residentBytes'])
        workers=worker_count(p,reserve)
    logging.getLogger(__name__).info('V5 fixture execution: %d workers',workers)
    refined=[]
    for faces in ordered_work(_refine_fixture_job,jobs(),workers,processes=True):
        refined.extend(faces)
    return refined


def _refine_fixture_job(job):
    support,proposals,p,bounds=job
    faces=refine_fixture_faces(support,proposals,p)
    if bounds is None:
        return faces
    lower,upper=bounds
    owned=support[np.all((support>=lower)&(support<upper),axis=1)]
    point_index=PointBoundsIndex(owned) if len(faces)>4 else None
    refined=[]
    for face in faces:
        # Core bounds own counts, not a clip of observed grid cells.
        face["coreBounds"]=[lower.tolist(),upper.tolist()]
        evidence=owned[fixture_mask(owned,[face],p,point_index=point_index)]
        if not len(evidence):continue
        local=(evidence-np.asarray(face["origin"]))@np.asarray(face["axes"]).T
        face["occupiedCells"]=unique_integer_rows(np.floor(local/face["gridSize"]).astype(np.int64)).tolist()
...
def classify(points,analysis,features=None,noise=None):
    ...
    point_index=PointBoundsIndex(points) if len(surfaces)>4 else None
    ...
    for face in surfaces:
        rows=(fixture_rows(points,face,p,point_index=point_index) if point_index is not None
              else np.flatnonzero(fixture_mask(points,[face],p)))
        if not len(rows):continue
        normal=_canonical_face_normal(face['normal'])
        residual=np.abs((points[rows]-np.asarray(face['origin']))@np.asarray(face['normal']))
        evidence=.65+.33*face.get('confidence',.8)*np.exp(-.5*(residual/p.fixture_fit_distance)**2)
        aligned=np.abs(normals[rows]@normal)
        ...
        fixture[rows]=True
    for model in bolt_candidates(points,details.get('fixture',{}).get('bolts',[]),p):
        support=bolt_mask(points,[model],p,point_index=point_index)
...
def raw_chunk(ordinal,analysis):
    ...
    if ordinal not in runtime.label_chunks:
        with runtime.chunks.read(runtime.feature_chunks[ordinal]) as raw:
            ...
            runtime.chunks.save(path,source_index=raw["source_index"],scene_class=attrs.scene_class,
    ...
    with runtime.chunks.read(runtime.label_chunks[ordinal]) as saved:
...
def stage_features(ordinal,raw,analysis):
    ...
    if ordinal in runtime.updated_features:
        ...
        if path is not None:
            with runtime.chunks.read(path) as saved:
                source_index=raw['source_index']
                for stage in ('table','fixture'):
                    index_key=stage+'_source_index'
                    if index_key not in saved:continue
                    rows=np.searchsorted(source_index,saved[index_key])
                    for key in features:
                        features[key][rows]=saved[stage+'_'+key]
        return features
    payload=_boundary_updates(runtime,ordinal,raw,details)
    _save_boundary_updates(runtime,ordinal,payload)
    for stage in ('table','fixture'):
        if stage+'_source_index' not in payload:continue
        rows=np.searchsorted(raw['source_index'],payload[stage+'_source_index'])
        for key in features:features[key][rows]=payload[stage+'_'+key]
    return features


def _save_boundary_updates(runtime,ordinal,payload):
    if payload:
        path=runtime.path/f"updates-{ordinal:06d}.npz"
        runtime.chunks.save(path,**payload);runtime.update_chunks[ordinal]=path
    runtime.updated_features.add(ordinal)


def _boundary_updates(runtime,ordinal,raw,details):
    """Compute one original core's sparse updates without mutating shared state."""
    p=runtime.p
    xyz=raw["xyz"];source_index=raw['source_index'];core_noise=raw['noise'].astype(bool)
    lo,hi=runtime.feature_bounds[ordinal]
    support=runtime.store.query(lo-p.halo-p.detection_voxel_size,hi+p.halo+p.detection_voxel_size)
    valid=~runtime.masks(support)
    coordinates=support["xyz"]
    table=table_mask(coordinates,details.get("plane"),p)
    # Every core record is already in this complete, source-sorted halo.
    # Select its exact mask instead of evaluating all models on it again.
    core_rows=np.searchsorted(support['source_index'],source_index)
    core_table=table[core_rows]
    if np.all(core_table|core_noise):
        return {}
    ...
    core_fixture=fixture[core_rows]
    payload={}
    for name,excluded,newly in (("table",table,table),("fixture",table|fixture,fixture)):
        ...
        rows=np.flatnonzero(near&~core_excluded&~core_noise)
        if not len(rows):continue
        updates=multiscale(coordinates[valid&~excluded],xyz[rows],p)
        payload[name+"_source_index"]=source_index[rows]
        for key,value in updates.items():
            payload[name+"_"+key]=value
    return payload


def _boundary_job(job):
    runtime,ordinal,raw,details=job
    return ordinal,_boundary_updates(runtime,ordinal,raw,details)


def _prime_boundary_updates(analysis):
    runtime=analysis.resources
    if runtime.store.count<250_000:return
    pending=[i for i in range(len(runtime.feature_chunks)) if i not in runtime.updated_features]
    if not pending:return
    reserve=max(0,runtime.chunks.budget-runtime.chunks.stats['residentBytes'])
    workers=worker_count(runtime.p,reserve)
    if workers<=1:return
    logging.getLogger(__name__).info('V5 boundary execution: %d workers, %d cores',workers,len(pending))
    def jobs():
        for ordinal in pending:
            raw=runtime.chunks.read(runtime.feature_chunks[ordinal])
            yield runtime,ordinal,raw,analysis.data['algorithmDetails']
    for ordinal,payload in ordered_work(_boundary_job,jobs(),workers):
        # Only the coordinator accesses the LRU and publishes completed updates.
        _save_boundary_updates(runtime,ordinal,payload)
...
def transfer_labels(points,analysis):
    ...
    for key,rows in integer_row_groups(keys):
        key=tuple(map(int,key))
        cached=runtime.transfer_cache.get(key)
        if cached is not None:
            tree,values,source_candidates=cached
        else:
            ...
            candidate_bytes=sys.getsizeof(source_candidates)+sum(
                sys.getsizeof(k)+sys.getsizeof(v)+sum(sys.getsizeof(i) for i in v)
                for k,v in source_candidates.items())
            # Include the KD-tree's coordinate/index buffers and a conservative
            # per-node allowance, not just the label columns.
            size=source.nbytes+tree.indices.nbytes+tree.size*128+candidate_bytes+sum(v.nbytes for v in values.values())
            runtime.transfer_cache.save(key,(tree,values,source_candidates),size)
    ...
    analysis.data.setdefault('diagnostics',{}).update(runtime.cache_diagnostics())
...
def finalize_raw_ownership(analysis):
    ...
    # A caller may already have projected display points. Changed ownership
    # invalidates all derived neighborhood indexes and their label columns.
    if hasattr(runtime,'transfer_cache'):runtime.transfer_cache.clear()
    _prime_boundary_updates(analysis)
    ...
    for ordinal,path in list(runtime.label_chunks.items()):
        with runtime.chunks.read(path) as raw:labels={k:raw[k] for k in raw.files}
        ...
        runtime.chunks.save(path,**labels)
...
def analyze(context,p):
    ...
    try:
        ...
        fixture_timings={"boundaryFeaturesElapsedS":time.perf_counter()-start}
        part_start=time.perf_counter()
        surfaces=detect_fixtures(residual,f,p,plane)
        fixture_timings["detectionElapsedS"]=time.perf_counter()-part_start
        logging.getLogger(__name__).info('V5 fixture proposals: %d faces in %.2f s',len(surfaces),fixture_timings['detectionElapsedS'])
        part_start=time.perf_counter()
        surfaces=refine_fixture_support(runtime,plane,surfaces,p)
        fixture_timings["rawRefinementElapsedS"]=time.perf_counter()-part_start
        logging.getLogger(__name__).info('V5 fixture refinement: %d faces in %.2f s',len(surfaces),fixture_timings['rawRefinementElapsedS'])
        part_start=time.perf_counter()
        bolts,bolt_diagnostic=detect_bolts(residual,f,surfaces,p)
        fixture_timings["boltDetectionElapsedS"]=time.perf_counter()-part_start
        part_start=time.perf_counter()
        fixture=fixture_mask(residual,surfaces,p)|bolt_mask(residual,bolts,p)
        fixture_timings["maskElapsedS"]=time.perf_counter()-part_start
        stages.append({"name":"fixtures","inputPointCount":len(residual),"confirmedPointCount":int(fixture.sum()),"candidateCount":len(surfaces),
                       "boltCount":len(bolts),"boltDiagnostics":bolt_diagnostic,"elapsedS":time.perf_counter()-start,**fixture_timings})
        ...
        data['diagnostics'].update(runtime.cache_diagnostics())
        return result
    except Exception:
        runtime.close();raise
...
def export_sidecars(directory,analysis):
    ...
    for ordinal,path in enumerate(runtime.feature_chunks):
        with runtime.chunks.read(path) as raw:
            ...
            # Keep the authoritative in-memory label columns for display transfer.
            if ordinal in runtime.update_chunks:
                update_name=f"updates-{ordinal:06d}.npz"
                with runtime.chunks.read(runtime.update_chunks[ordinal]) as update:
                    np.savez_compressed(feature_dir/update_name,**update)
    ...
    analysis.data['diagnostics'].update(runtime.cache_diagnostics())
