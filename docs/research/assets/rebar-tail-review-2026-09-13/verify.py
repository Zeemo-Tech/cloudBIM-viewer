...
assert report['version'] == 'design-guided-instances-v17-local-tail-review'
...
for n in names:
    np.testing.assert_array_equal(arrays[n], np.load(ROOT/f'.cloudbim/tail-fix-20260913/current/after-{n}.npy',mmap_mode='r'))
...
for decision in report['designReview']['overlengthTailFilter']['decisions']:
    ...
    assert item['shapeReview']['observedLengthM']+1e-9 >= decision['designLengthM']
    assert item['lengthM']+1e-9 >= decision['designLengthM']
