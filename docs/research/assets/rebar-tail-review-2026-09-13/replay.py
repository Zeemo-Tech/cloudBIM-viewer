...
parser.add_argument('--run-id', default='20260912T113748-23c11a16')
parser.add_argument('--current', action='store_true')
options = parser.parse_args()
RUN = ROOT/'backend/data/assets/95b6b41c5857d9eb3407b155/pointcloud-steps'/options.run_id
OUT = ROOT/'.cloudbim/tail-fix-20260913'
if options.current: OUT = OUT/'current'
OUT.mkdir(parents=True, exist_ok=True)
old_path = OUT/'baseline_tails.py'
old_path.write_bytes(subprocess.check_output(['git', 'show', '169d1a0:services/mesh-service/algorithms/rebar_overlength_tails.py'], cwd=ROOT))
